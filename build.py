...
if os.path.exists(_pk_path):
    POKEMON = set(json.load(open(_pk_path)))
...
# 2) ALL PSN-valid 3-char combos (26 x 38^2 = 37,544; class-verified reserved, see class3.json)
#    plus ALL 4-letter combos (26^4 = 456,976) — the full space, minus digit/underscore 4-char junk
import itertools, string
letters = string.ascii_lowercase
CH3 = letters + '0123456789_-'
for a in letters:
    for b in CH3:
        for c in CH3:
            add(a + b + c)
...
# 5) numbers category (must still start with a letter)
NUM_TOKENS = """v1 v2 v3 v4 k9 r8 m3 m5 x1 ps1 ps2 ps3 ps4 ps5 gt3 gt4 mk1 mk2 mk3 ssj2 ssj3 gta5 gta6 mw2 mw3 bo2 bo3 f1x
x100 x360 x500 x007 v100 v12 v24 n7 z3 z4 a1 b2 c3 d4 e5 f6 g7 h8 i9 j1 k2 l3 m4 n5 o6 p7 q8 r9 s1 t2 u3 w4 y5 z6"""
for t in NUM_TOKENS.split():
    add(t)
for i in range(0, 10):
    add(f"x{i}{i}")
# a curated sprinkle of word+digit variants (the interesting ones, not every permutation)
DIGIT_WORDS = "ace pro god og king wolf zero ghost dark storm nova echo blaze legend sniper shadow".split()
for w in DIGIT_WORDS:
    add(w + "1"); add(w + "2"); add(w + "7"); add(w + "69")
DIGIT_NAMES = "max leo kai sam alex".split()
for w in DIGIT_NAMES:
    add(w + "1"); add(w + "5")
# letter + 2 digits (deterministic sample of the cleanest)
letters_d = sorted(string.ascii_lowercase, key=lambda c: fnv("d" + c))
for a in letters_d:
    pairs = sorted((d + e for d in '0123456789' for e in '0123456789'), key=lambda p: fnv(a + p))[:4]
    for p in pairs:
        add(a + p)
...
# priority sweep queue: Pokemon first, then everything len>=4 by rarity (3-char is class-answered)
queue = sorted([n for n in POKEMON], key=lambda n: (-score_of(n, mask_for(n)), n))
queue += [r[0] for r in rows if len(r[0]) >= 4 and not (r[0] in POKEMON)]
...
<header>
  ...
  <p>A Spell-style catalogue of PSN Online ID candidates — browse, search and filter every handle, each scored with a 0–99 rarity rating.
     ...
     Any valid 3–16-character ID is searchable. Word forms are tagged <b>Final</b> (whole word — <span class="mono">lonely</span>) or
...
function buildChips(){
  ...
  const hc=$("charchips"); const CL=["Letters only","Has digits","Has underscore","Has hyphen"]; const CK=["letters","digits","underscore","hyphen"];
