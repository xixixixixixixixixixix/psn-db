...
def add(name, random_flag=False):
    ...
    # Sony class-reserves 3 and 4 character IDs (HTTP 406). Listed catalogue is 5+.
...
# 2) 3-char and 4-char are Sony class-reserved (406). Not listed. Search/check
#    short-circuit in the worker + UI. Scanner starts at aaaaa.
...
# 6) repeating characters / palindromes / doubled syllables (5+ only; add() drops shorter)
for ch in letters:
    add(ch * 5)           # aaaaa… — 3/4-char repeats are class-reserved
...
# 7) letter patterns (qwerty / alphabet runs) — 5+ only
...
# Letter-only ids are generated by server.py (aaaaa → aaaab → … up to 16).
# 3-char and 4-char are class-reserved and never queued.
queue = sorted((r[0] for r in rows
                if len(r[0]) >= 5 and not r[0].isalpha() and not any(c.isdigit() for c in r[0])),
...
.livebar{display:none;margin:12px 24px 0;padding:10px 14px;border:1px solid rgba(124,200,150,.4);borders:3px;background:rgba(18,28,20,.88);backdrop-filter:blur(8px);font-size:13px;align-items:center;gap:10px;flex-wrap:wrap}
...
<header>
  ...
  <p>One place to browse, score and verify usernames across platforms.
     ...
     Letters only (plus <span class="mono">_</span>/<span class="mono">-</span>), no numbers. PlayStation listing is <b>5+ characters</b> — Sony class-reserves every 3 and 4 character ID. Taken names drop off the list but stay on record.</p>
...
<main>
  <aside>
    <div class="card"><h3>Rarity</h3><div class="chips" id="tierchips"></div></div>
    ...
    <div class="card"><h3>Length</h3>
      <div class="lenrow"><input id="lenmin" type="number" min="5" max="16" value="5"> to <input id="lenmax" type="number" min="5" max="16" value="16"></div>
    </div>
  </aside>
...
const WHY = ["-", "Available", "Taken (account exists)", "Blocked word (Sony)", "Reserved — 3 and 4 character IDs are no longer issued", "Reserved by Sony (policy, trademark or a previous holder)"];
...
const state = { q:"", tiers:new Set(), avails:new Set(), cats:new Set(), chars:new Set(), gens:new Set(),
                lenMin:5, lenMax:16, favOnly:false, verifiedOnly:false, sort:"score_desc", shown:60 };
const PAGE = 60;
...
function filtered(){
  ...
  return ALL.filter(r=>{
    ...
    if(platform==="psn" && r.len < 5) return false;  // Sony class-reserves 3–4
...
function emptyRow(){
  ...
  if(platform==="psn" && /^[a-z][a-z0-9_\-]{2,15}$/.test(name) && name.length<=4){
    return `<tr><td colspan="7" style="text-align:center;color:var(--muted);padding:30px">“${name}” is <b style="color:var(--red)">reserved</b> — Sony no longer issues 3 or 4 character IDs (class-wide 406). Not listed.</td></tr>`;
  }
...
function renderStats(){
  ...
  $("stats").innerHTML =
    ...
    `<div class="stat">${availLbl} <b>${verAv.toLocaleString()}</b></div>`+
...
$("reset").onclick = ()=>{
  ...
  state.lenMin=5; state.lenMax=16; state.favOnly=false; state.verifiedOnly=false; state.gens.clear(); state.sort="score_desc"; state.shown=PAGE;
  $("search").value=""; $("sort").value="score_desc"; $("lenmin").value=5; $("lenmax").value=16;
  $("favbtn").classList.remove("on"); buildChips(); render();
};

/* length */
$("lenmin").addEventListener("change", e=>{ state.lenMin=Math.max(5,Math.min(16,+e.target.value||5)); state.shown=PAGE; render(); });
$("lenmax").addEventListener("change", e=>{ state.lenMax=Math.max(5,Math.min(16,+e.target.value||16)); state.shown=PAGE; render(); });
...
function applyLive(name, j){
  ...
  if(platform==="psn" && name.length<=4){
    const r = nameFind(name); if(r && !r.dropped) r.dropped = 1;
    TAKENIDX.set(name, {w:4, ts:j.ts||Math.floor(Date.now()/1000)});
    return null;
  }
...
function rehydrateLive(){
  for(const name of Object.keys(liveStore)){
    if(/\d/.test(name) || name.length<5){ delete liveStore[name]; continue; }
...
function maybeLive(){
  ...
  if(platform==="psn" && name.length<=4){
    livebar(`<span>⚡ <b class="mono">${name}</b> is class-reserved — Sony no longer issues 3 or 4 character IDs.</span><span class="badge b-taken"><span class="dot"></span>Reserved</span>`, true);
    return;
  }
  const r = nameFind(name);
